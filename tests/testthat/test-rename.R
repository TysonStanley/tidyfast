...
test_that("rename_at(): .vars works with select helpers in list()", {
  df <- data.table(x_start = c(1,1,1), end_x = c(2,2,2), z = c("a", "a", "b"))
  df <- df %>%
    dt_rename_at(list(dt_starts_with("x")), function(.x) paste0(.x, "_append"))
...
test_that("mutate_at() works for range selection", {
  df <- data.table(x = c(1,1,1), y = c(2,2,2), z = c("a", "a", "b"))
  df <- df %>%
    dt_rename_at(list(x:y), function(.x) paste0(.x, "_append"))
